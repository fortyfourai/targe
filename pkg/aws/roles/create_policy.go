		...
		if msg.String() == "enter" {
			if m.done != nil && *m.done {
				...
				if m.createTerraform != nil && *m.createTerraform {
					terraformSnippet := generateTerraformSnippet(policy.Id, string(policyJson), m.controller.State.GetRole().Arn)
...
func generateTerraformSnippet(policyID, policyDoc, roleArn string) string {
	return fmt.Sprintf(`
...
`, policyID, policyID, policyDoc, roleArn, policyID)
}
